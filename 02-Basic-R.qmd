...
A quarto document is a pure text file that you can open in any text editor (e.g. Notepad, Emacs, Kate, etc.). It consists of two parts: the header at the top of the file. The header starts with three dashes on a separate line and ends with three dashes on a separate line. Between the dashes you can specify different options in a `yml` format such as the author and title of the document, but these will be generally irrelevant for our course and we will ignore them.

::: callout-note
## Do not open the R Markdown file in MS Word or LibreOffice

MS Word and OpenOffice/LibreOffice are not simple text editors. If you open a R Markdown file in MS Word or LibreOffice, it will add formatting to the text and the file will no longer be a plain text file. This will cause problems when you try to compile the document.
:::
...
You can find the complete list of shortcuts under `Help` -\> `Keyboard Shortcuts Help`.
:::
