...
```{ojs}
//| echo: false
viewof evidenceSequence = slider({
  min: 0, 
  max: 4, 
  step: 1, 
  value: 3, 
  title: "sequence of draws ",
})
```

```{ojs}
//| echo: false
viewof conjecture_input = Inputs.checkbox(["blue", "blue", "blue", "blue"], {label: "Evidence"})
conjectures = [conjecture_input.map((c, i) => c == "blue" ? 1 : 0)]
```

```{ojs}
//| echo: false
viewof evidenceInput = Inputs.checkbox(["blue", "white", "white"], {label: "Evidence"})
...
inlineDraw = function(data, width = 70) {
...
conjecture = function({blue = 1, white = 3}) {
